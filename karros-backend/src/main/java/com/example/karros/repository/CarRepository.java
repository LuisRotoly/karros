...
public interface CarRepository extends CrudRepository<CarModel,Integer> {
    ...
    @Query(value = "SELECT c FROM CarModel c WHERE c.isDeleted = false ORDER BY c.amount")
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE CarModel c SET c.isDeleted = true, c.deletedAt = :deletedAt WHERE c.id = :id")
    void setDeletedFlagTrue(Long id, LocalDateTime deletedAt);
}
